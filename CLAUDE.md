...
python manage.py seed_microcruz        # BORRA y recarga (recomendado): 147 líneas (Microcruz) + ~169 paradas físicas reales (OSM)
...
- `seed_microcruz` ([commands/seed_microcruz.py](backend/transit_app/management/commands/seed_microcruz.py)) — **el más completo y el recomendado**: combina las ~147 líneas reales (con trazado ida/vuelta) de la API pública de Microcruz (`https://microcruz.tel.bo`) con las paradas físicas reales de OSM (nodos `highway=bus_stop`/`public_transport=platform`, es decir con caseta/señalética real — NO los ~2165 "puntos" propios de Microcruz, que son nodos de su grafo de calles para el buscador de rutas, ni paradas generadas artificialmente por espaciado: ambas alternativas se probaron y se descartaron). Vincula cada parada a la ruta más cercana con `LineLocatePoint` + `dwithin`, misma lógica que `seed_osm`.
- La búsqueda de transbordos en `planTrip` ([schema.py](backend/transit_app/schema.py)) es combinatoria (hasta 12x12 pares de rutas, comparando todas sus paradas entre sí) y se salta si ya hay 3+ directas encontradas, para que responda en segundos.
